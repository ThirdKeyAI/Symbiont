...
Symbiont v1.9.0は最新の安定版リリースであり、本番レベルの機能を備えた完全なAIエージェントフレームワークを提供します：

- **ToolClad統合**: マニフェストの読み込み、引数バリデーション、HTTP/MCPプロキシバックエンド、シークレット注入、セッション/ブラウザエグゼキューターを備えた宣言的ツールコントラクト
- **`symbi tools` CLI**: スコープ強制、Cedarポリシー生成、ToolCladマニフェスト用ホットリロードファイルウォッチャー
- **本番環境強化**: バウンドチャネル、ヘルスプローブ、シークレットTTL、Cedarポリシーリロード、監査エクスポート、レート制限
- **セキュリティ修正**: 重大なDoSベクター緩和、JWTバリデーション強化、環境変数リーク防止、サンドボックスガード改善
- **W3Cトレースペアレント伝播**: エージェント境界を越えたOpenTelemetry分散トレースコンテキスト伝播
