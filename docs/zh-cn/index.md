...
- **🤖 AI 助手插件**：为 [Claude Code](https://github.com/thirdkeyai/symbi-claude-code) 和 [Gemini CLI](https://github.com/thirdkeyai/symbi-gemini-cli) 提供一等治理插件，支持 Cedar 策略执行、SchemaPin 验证和审计轨迹

### 项目初始化（`symbi init`）

交互式项目脚手架，支持基于配置文件的模板。可从 `minimal`、`assistant`、`dev-agent` 或 `multi-agent` 配置文件中选择。支持可配置的 SchemaPin 验证模式和沙箱层级。内置智能体目录，可导入预构建的治理智能体。通过 `--no-interact` 支持 CI/CD 管线的非交互模式。

### 单智能体执行（`symbi run`）

直接从 CLI 运行任意智能体，无需启动完整运行时：

```bash
symbi run recon --input '{"target": "10.0.1.5"}'
```

加载智能体 DSL，设置带有云端推理的 ORGA 推理循环，执行后输出结果并退出。自动从 `agents/` 目录解析智能体名称。

### 智能体间通信治理

所有智能体间内置函数（`ask`、`delegate`、`send_to`、`parallel`、`race`）均通过 `CommunicationBus` 路由并进行策略评估。`CommunicationPolicyGate` 为智能体间调用强制执行 Cedar 风格的规则——控制哪些智能体可以通信，支持基于优先级的规则评估，并在策略违规时硬拒绝。消息经过密码学签名、加密和审计。
...
Symbiont v1.9.0 是最新的稳定版本，提供具有生产级功能的完整 AI 智能体框架：

- **ToolClad 集成**：声明式工具合约，支持清单加载、参数验证、HTTP/MCP 代理后端、密钥注入和会话/浏览器执行器
- **`symbi tools` CLI**：作用域强制、Cedar 策略生成和 ToolClad 清单热重载文件监视器
- **生产环境加固**：有界通道、健康探针、密钥 TTL、Cedar 策略重载、审计导出和速率限制
- **安全修复**：关键 DoS 向量缓解、JWT 验证加固、环境变量泄漏防护和沙箱守卫改进
- **W3C Traceparent 传播**：跨智能体边界的 OpenTelemetry 分布式追踪上下文传播
