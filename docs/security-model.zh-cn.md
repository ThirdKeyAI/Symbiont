...
- **推理循环集成**：`CedarGate` 实现了 `ReasoningPolicyGate` trait，在执行前针对 Cedar 策略评估每个提议的操作
- **审计轨迹**：所有 Cedar 策略决策都以完整上下文记录

```rust
use symbi_runtime::reasoning::cedar_gate::CedarGate;

// 加载 Cedar 策略并在推理循环中评估操作
...
| 级别 | 操作 | 描述 |
|------|------|------|
| **严重** | 扫描失败 | 主动利用模式（反向 shell、代码注入） |
| **高** | 扫描失败 | 凭据窃取、权限提升、进程注入 |
| **中等** | 警告 | 可疑但可能合法（下载器、符号链接） |
| **警告** | 警告 | 低风险指标（环境文件引用、chmod） |
| **信息** | 记录 | 信息性发现 |
