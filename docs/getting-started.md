...
This guide will walk you through setting up Symbiont and creating your first AI agent.
{: .fs-6 .fw-300 }
...
Before getting started with Symbiont, ensure you have the following installed:
...
Let's create a simple data analysis agent to understand the basics of Symbiont.

### 1. Create Agent Definition

Create a new file `my_agent.symbiont`:

```symbiont
metadata {
    version = "1.0.0"
    author = "your-name"
    description = "My first Symbiont agent"
}
...
cargo run --bin symbiont_parser my_agent.symbiont

# Run the agent in the runtime
cargo run --example basic_agent -- --agent my_agent.symbiont
```
...
The Symbiont DSL has several key components:
...
export SYMBIONT_RUNTIME_MODE=development
...
Create a `symbiont.toml` configuration file:
...
collection_name = "symbiont_knowledge"
```
...
cargo run --bin symbiont_parser your_agent.symbiont
...
```bash
# Enable debug logging
export RUST_LOG=symbiont=debug

# Run with detailed output
cargo run --example basic_agent 2>&1 | tee debug.log
```
...
Now that you have Symbiont running, explore these advanced topics:
...
Ready to build something amazing? Start with our [example projects](https://github.com/thirdkey/symbiont/tree/main/runtime/examples) or dive into the [complete specification](/specification).