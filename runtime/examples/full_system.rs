...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let agents = vec![
        create_agent_config("web_scraper", ExecutionMode::Scheduled { 
            interval: Duration::from_secs(3600) 
        }, SecurityTier::Tier2, Priority::Normal),
        create_agent_config("data_processor", ExecutionMode::Persistent, 
            SecurityTier::Tier1, Priority::High),
        create_agent_config("notification_service", ExecutionMode::EventDriven, 
            SecurityTier::Tier1, Priority::Low),
    ];
    ...
    for config in agents {
        ...
        let _allocation = resource_manager.allocate_resources(
            config.id, 
            requirements
        ).await?;
    ...
    for config in &agent_configs {
        ...
        let initialized_id = lifecycle_controller.initialize_agent(config.clone()).await?;
        ...
        println!("  ✓ Agent {} ready for operation (would start in real system)", config.name);
    ...
    if initialized_agent_ids.len() >= 2 && agent_configs.len() >= 2 {
        ...
        comm_bus.subscribe(receiver_id, "data_updates".to_string()).await?;
    ...
    if let Some(&agent_id) = initialized_agent_ids.first() {
        ...
        println!("  Error stats: {} total, {} recent", stats.total_errors, stats.recent_errors);
    }
    ...
    println!("Available CPU cores: {}", resource_status.available_cpu_cores);
    
    // Get system error stats
    let error_stats = error_handler.get_system_error_stats().await;
    println!("System errors: {} total, {} recent", error_stats.total_errors, error_stats.recent_errors);
