...
use symbiont_runtime::context::manager::{StandardContextManager, ContextManagerConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            Ok(response) => {
                ...
                if !response.sources_used.is_empty() {
                    ...
                    for (j, citation) in response.sources_used.iter().enumerate() {
                        println!("  {}. {} (relevance: {:.2})", 
                               j + 1, citation.title, citation.relevance_score);
    ...
    for (i, doc) in retrieved_docs.iter().enumerate() {
        println!("  {}. {} (type: {:?})", i + 1, doc.title, doc.metadata.document_type);
    }
    
    // Step 4c: Document Ranking
    println!("\n--- Step 3: Document Ranking ---");
    let ranked_docs = rag_engine.rank_documents(retrieved_docs, &analyzed_query).await?;
    println!("Ranked {} documents by relevance", ranked_docs.len());
    for (i, ranked_doc) in ranked_docs.iter().enumerate() {
        println!("  {}. {} (score: {:.3})", 
               i + 1, ranked_doc.document.title, ranked_doc.relevance_score);
        let factors = &ranked_doc.ranking_factors;
        println!("     Semantic: {:.2}, Keywords: {:.2}, Recency: {:.2}, Authority: {:.2}", 
               factors.semantic_similarity, factors.keyword_match, 
               factors.recency_score, factors.authority_score);
    }
    
    // Step 4d: Context Augmentation
    println!("\n--- Step 4: Context Augmentation ---");
    let augmented_context = rag_engine.augment_context(&analyzed_query, ranked_docs).await?;
    ...
    println!("Generated response ({} characters)", generated_response.content.len());
    println!("Confidence: {:.2}", generated_response.confidence);
    println!("Sources consulted: {}", generated_response.metadata.sources_consulted);
    println!("Generation time: {:?}", generated_response.metadata.generation_time);
    
    // Step 4f: Response Validation
    println!("\n--- Step 6: Response Validation ---");
    let validation_result = rag_engine.validate_response(&generated_response, agent_id).await?;
    ...
    println!("Policy violations: {}", validation_result.policy_violations.len());
    ...
    println!("Validation pass rate: {:.2}%", stats.validation_pass_rate * 100.0);
