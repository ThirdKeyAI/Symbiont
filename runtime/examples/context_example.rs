...
use symbiont_runtime::context::manager::{StandardContextManager, ContextManagerConfig, ContextManager};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let working_memory_updates = vec![
        ...
        MemoryUpdate {
            ...
            data: json!(SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)?.as_secs()),
        },
    ];
    
    context_manager.update_memory(agent1_id, working_memory_updates).await?;
    ...
    let fact_id = context_manager.add_knowledge(agent1_id, Knowledge::Fact(fact)).await?;
    ...
    let procedure_id = context_manager.add_knowledge(agent1_id, Knowledge::Procedure(procedure)).await?;
    println!("✓ Added procedure to Agent 1's knowledge base: {}", procedure_id);
    ...
    let pattern_id = context_manager.add_knowledge(agent1_id, Knowledge::Pattern(pattern)).await?;
    println!("✓ Added pattern to Agent 1's knowledge base: {}", pattern_id);

    // Step 5: Search Knowledge Base
    println!("\n=== Knowledge Search ===");
    
    let search_results = context_manager.search_knowledge(agent1_id, "user preferences", 5).await?;
    println!("✓ Found {} knowledge items for 'user preferences'", search_results.len());
    
    for (i, item) in search_results.iter().enumerate() {
        println!("  {}. {} (type: {:?}, confidence: {:.2})", 
               i + 1, item.content, item.knowledge_type, item.confidence);
    }
    ...
    let query_results = context_manager.query_context(agent1_id, context_query).await?;
    println!("✓ Context query returned {} items", query_results.len());
    
    for (i, item) in query_results.iter().enumerate() {
        println!("  {}. {} (relevance: {:.2})", 
               i + 1, item.content, item.relevance_score);
    }
    ...
    context_manager.share_knowledge(
        agent1_id, 
        agent2_id, 
        fact_id, 
        AccessLevel::Public
    ...
    println!("✓ Agent 2 has access to {} shared knowledge items", shared_knowledge.len());
    
    for (i, shared_ref) in shared_knowledge.iter().enumerate() {
        println!("  {}. Knowledge from Agent {} (trust: {:.2})", 
               i + 1, shared_ref.source_agent, shared_ref.trust_score);
    }

    // Step 8: Context Retrieval
    println!("\n=== Context Retrieval ===");
    
    let retrieved_context = context_manager.retrieve_context(agent1_id, Some(session1_id)).await?;
    
    if let Some(context) = retrieved_context {
        println!("✓ Retrieved context for Agent 1");
        println!("  Session ID: {}", context.session_id);
        println!("  Working memory variables: {}", context.memory.working_memory.variables.len());
        println!("  Knowledge facts: {}", context.knowledge_base.facts.len());
        println!("  Knowledge procedures: {}", context.knowledge_base.procedures.len());
        println!("  Knowledge patterns: {}", context.knowledge_base.learned_patterns.len());
    ...
    let layered_context_id = context_manager.store_context(agent2_id, layered_context).await?;
    println!("✓ Stored layered context for Agent 2: {}", layered_context_id);
    
    // Retrieve and display the layered context
    let retrieved_layered = context_manager.retrieve_context(agent2_id, Some(session2_id)).await?;
    if let Some(context) = retrieved_layered {
        println!("✓ Retrieved layered context:");
        println!("  Working memory goals: {}", context.memory.working_memory.active_goals.len());
        ...
        println!("  Semantic concepts: {}", context.memory.semantic_memory.len());
        println!("  Conversation history: {}", context.conversation_history.len());
        
        // Show semantic relationships
        if let Some(semantic_item) = context.memory.semantic_memory.first() {
            println!("  Concept '{}' has {} relationships", 
                   semantic_item.concept, semantic_item.relationships.len());
            for rel in &semantic_item.relationships {
                println!("    {} -> {} (strength: {:.2})", 
                       rel.relation_type.to_string(), rel.target_concept, rel.strength);
            }
        }
    }
