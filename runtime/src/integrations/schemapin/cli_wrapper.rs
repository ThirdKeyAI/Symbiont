...
use super::types::{SchemaPinConfig, SchemaPinError, VerificationResult, VerifyArgs, SigningResult, SignArgs};
...
impl SchemaPinCliWrapper {
    ...
    fn parse_verification_result(&self, json_output: &str) -> Result<VerificationResult, SchemaPinError> {
        serde_json::from_str(json_output)
            .map_err(|e| SchemaPinError::JsonParsingFailed {
    ...
    fn parse_signing_result(&self, json_output: &str) -> Result<SigningResult, SchemaPinError> {
        serde_json::from_str(json_output)
            .map_err(|e| SchemaPinError::JsonParsingFailed {
                reason: format!("Failed to parse signing JSON: {}. Output: {}", e, json_output),
            })
    }
}
...
impl SchemaPinCli for SchemaPinCliWrapper {
    async fn verify_schema(&self, args: VerifyArgs) -> Result<VerificationResult, SchemaPinError> {
        ...
        if !args.public_key_url.starts_with("http://") && !args.public_key_url.starts_with("https://") {
...
mod tests {
    ...
    fn test_config_creation() {
        ...
        assert_eq!(cli.config.environment.get("TEST_VAR"), Some(&"test_value".to_string()));
    }
