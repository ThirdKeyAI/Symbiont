...
use super::types::{
    SchemaPinError, VerificationResult, VerifyArgs, SigningResult, SignArgs, SignatureInfo
};
...
impl SchemaPinClient for NativeSchemaPinClient {
    async fn verify_schema(&self, args: VerifyArgs) -> Result<VerificationResult, SchemaPinError> {
        ...
        let key_id = calculate_key_id(&public_key_pem)
            .map_err(|e| SchemaPinError::IoError {
    ...
    async fn sign_schema(&self, args: SignArgs) -> Result<SigningResult, SchemaPinError> {
        ...
        let signature = sign_data(&private_key_pem, &schema_data)
            .map_err(|e| SchemaPinError::SigningFailed {
        ...
        let output_path = args.output_path.unwrap_or_else(|| {
            format!("{}.signed", args.schema_path)
        });
