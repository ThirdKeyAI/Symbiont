...
impl ResourcePool {
    ...
    pub fn allocate(&mut self, agent_id: AgentId, limits: &ResourceLimits) -> Option<ResourceAllocation> {
    ...
    pub fn get_utilization(&self) -> ResourceUtilization {
        ...
        let cpu_utilization = if self.total_cpu_cores > 0 {
            (self.total_cpu_cores as f32 - self.available_cpu_cores as f32) / self.total_cpu_cores as f32
        } else {
            0.0
        };
