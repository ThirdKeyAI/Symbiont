...
mod tests {
    ...
    async fn test_session_creation() {
        ...
        let context = manager.retrieve_context(agent_id, Some(session_id)).await.unwrap();
    ...
    async fn test_memory_operations() {
        ...
        assert!(manager.update_memory(agent_id, memory_updates).await.is_ok());
    }
    ...
    async fn test_knowledge_operations() {
        ...
        let _knowledge_id = manager.add_knowledge(agent_id, Knowledge::Fact(fact)).await.unwrap();
        
        // Test searching knowledge
        let results = manager.search_knowledge(agent_id, "test", 10).await.unwrap();
