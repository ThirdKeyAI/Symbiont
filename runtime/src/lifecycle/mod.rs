...
impl DefaultLifecycleController {
    ...
    async fn process_lifecycle_event(
        ...
        match event {
            LifecycleEvent::StateTransition { agent_id, from_state, to_state } => {
                if let Some(agent) = agents.write().get_mut(&agent_id) {
                    if state_machine.is_valid_transition(&from_state, &to_state) {
                        ...
                        tracing::info!("Agent {} transitioned from {:?} to {:?}", agent_id, from_state, to_state);
                    } else {
                        tracing::error!("Invalid state transition for agent {}: {:?} -> {:?}", agent_id, from_state, to_state);
                    }
                }
            }
    ...
    async fn monitor_agent_states(
        ...
        
        {
            ...
            for (agent_id, agent) in agents_read.iter() {
                ...
                if time_in_state > Duration::from_secs(300) { // 5 minutes
                    match agent.state {
...
impl LifecycleController for DefaultLifecycleController {
    async fn initialize_agent(&self, config: AgentConfig) -> Result<AgentId, LifecycleError> {
        ...
        if agents_count >= self.config.max_agents {
            return Err(LifecycleError::ResourceExhausted {
                reason: format!("Agent slots exhausted: {} / {}", agents_count, self.config.max_agents),
            });
        }
    ...
    async fn start_agent(&self, agent_id: AgentId) -> Result<(), LifecycleError> {
        ...
        if !self.state_machine.is_valid_transition(&current_state, &AgentState::Running) {
    ...
    async fn suspend_agent(&self, agent_id: AgentId) -> Result<(), LifecycleError> {
        ...
        if !self.state_machine.is_valid_transition(&current_state, &AgentState::Suspended) {
            return Err(LifecycleError::InvalidStateTransition {
                from: format!("{:?}", current_state),
...
impl AgentStateMachine {
    pub fn new() -> Self {
        ...
        valid_transitions.insert(AgentState::Initializing, vec![AgentState::Ready, AgentState::Failed]);
        valid_transitions.insert(AgentState::Ready, vec![AgentState::Running, AgentState::Suspended, AgentState::Terminating]);
        valid_transitions.insert(AgentState::Running, vec![AgentState::Suspended, AgentState::Completed, AgentState::Failed, AgentState::Terminating]);
        valid_transitions.insert(AgentState::Suspended, vec![AgentState::Running, AgentState::Terminating]);
        valid_transitions.insert(AgentState::Completed, vec![AgentState::Terminating]);
        valid_transitions.insert(AgentState::Failed, vec![AgentState::Initializing, AgentState::Terminating]);
...
mod tests {
    use super::*;
    use crate::types::{ExecutionMode, SecurityTier, ResourceLimits, Priority};
    use std::collections::HashMap;
    ...
    async fn test_agent_initialization() {
        let controller = DefaultLifecycleController::new(LifecycleConfig::default()).await.unwrap();
    ...
    async fn test_state_transitions() {
        ...
        controller.send_event(LifecycleEvent::StateTransition {
            agent_id,
