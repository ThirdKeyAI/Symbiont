...
impl LoadBalancer {
    ...
    pub fn new(strategy: LoadBalancingStrategy) -> Self {
        ...
        Self {
            strategy,
            resource_pool: Arc::new(RwLock::new(ResourcePool::new(total_memory, total_cpu_cores))),
...
impl AllocationHistory {
    ...
    fn record_allocation(&mut self, agent_id: AgentId, duration: std::time::Duration) {
        ...
        self.recent_allocations.insert(agent_id, std::time::SystemTime::now());
...
mod tests {
    ...
    async fn test_resource_deallocation() {
        ...
        let allocation = load_balancer.allocate_resources(&requirements).await.unwrap();
