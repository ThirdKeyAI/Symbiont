...
async fn test_system_status_reporting() {
    ...
    assert_eq!(
        status.total_network_bandwidth,
        config.total_network_bandwidth
    );
...
async fn test_resource_violation_detection() {
    ...
    let _ = resource_manager
        .update_usage(agent_id, excessive_usage)
        .await;
    ...
    let violations = resource_manager
        .check_resource_violations(agent_id)
        .await
        .unwrap();
    assert!(
        !violations.is_empty(),
        "Expected resource violations to be detected"
    );
...
async fn test_resource_manager_shutdown() {
    ...
    let post_shutdown_result = resource_manager
        .allocate_resources(AgentId::new(), create_test_requirements(128, 0.25))
        .await;
...
async fn test_duplicate_allocation_prevention() {
    ...
    let first_result = resource_manager
        .allocate_resources(agent_id, requirements.clone())
        .await;
    assert!(first_result.is_ok());

    // Second allocation for same agent should fail
    let second_result = resource_manager
        .allocate_resources(agent_id, requirements)
        .await;
    assert!(second_result.is_err());

    match second_result {
        Err(ResourceError::AllocationExists {
...
async fn test_resource_priority_handling() {
    ...
    let normal_result = resource_manager
        .allocate_resources(normal_priority_agent, normal_priority_requirements)
        .await;

    // Then try high priority allocation
    let high_result = resource_manager
        .allocate_resources(high_priority_agent, high_priority_requirements)
        .await;

    // Both should work within available system resources
    if normal_result.is_ok() {
        let _ = resource_manager
            .deallocate_resources(normal_priority_agent)
            .await;
    }
    if high_result.is_ok() {
        let _ = resource_manager
            .deallocate_resources(high_priority_agent)
            .await;
    }
}
