...
> **セキュリティデフォルト（v1.13.0 監査後）。** このバージョン以降：
> - 推論ループのポリシーゲート（`symbi up`、`symbi run`）はデフォルトで**フェイルクローズ**です — ツール呼び出しおよびエージェント間委任は、明示的なポリシーバックエンドが配線されない限り拒否されます。従来の許可的デフォルトは `--insecure-allow-all` / `SYMBI_INSECURE_ALLOW_ALL=1` の背後に隔離され、目立つ stderr バナーが表示されます。ローカル開発時のみ使用してください。
> - JWT 検証器はアルゴリズム許可リストを強制します（非対称パスでは ES256/EdDSA、HMAC Webhook パスでは HS256）。RSA 署名の JWT は拒否され、これによりオペレーターが制御するすべてのパスで `rsa` クレートのタイミング攻撃アドバイザリ（RUSTSEC-2023-0071）が無効化されます。Microsoft Teams アダプターは Bot Framework の要件により依然として RS256 を使用しますが、そのサーフェスは MS 署名トークンに限定されています。
> - `composio` フィーチャーフラグ、`ComposioToolExecutor`、`symbiont_mcp add`/`list` CLI サブコマンド、SymbiBot 自動投稿機能は完全に削除されました — Composio は静的な許可リストや TLS ピンニングなしに LLM が供給したツール名をディスパッチしていました。詳細な根拠は `SECURITY_AUDIT.md` C3 を参照してください。代わりに独自の `ActionExecutor` を持ち込んでください。
> - `docker-compose.test.yml` は `SYMBIONT_API_TOKEN` を必須とし（`testtoken123` のデフォルトはなし）、`0.0.0.0` ではなく `127.0.0.1` にバインドします。`.env.example` が同梱されています。
> - 新しい環境変数は `docs/getting-started.md` に記載されています：`SYMBI_INSECURE_ALLOW_ALL`、`SYMBI_REJECT_LEGACY_API_KEYS`、`SYMBI_UNSAFE_NATIVE_SANDBOX`。削除されたもの：`SYMBIONT_ALLOW_NO_JWT_AUDIENCE`、`COMPOSIO_API_KEY`、`COMPOSIO_MCP_URL`。
>
> 完全な監査および脅威モデル：`SECURITY_AUDIT.md`。アウトオブバンドのオペレーター項目（PAT ローテーションなど）：`SECURITY-OPS.md`。
