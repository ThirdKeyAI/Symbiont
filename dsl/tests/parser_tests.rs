...
use dsl::{parse_dsl, print_ast, extract_metadata};
...
mod parser_tests {
    ...
    policy TestPolicy {
        ...
        assert!(found_agent, "Should find an agent_definition node in the AST");
    }
    ...
    capabilities: [test]
        ...
        assert!(metadata.is_empty(), "Should extract no metadata when none present");
    }
    ...
    // Missing closing brace"#;
        ...
        assert!(result.is_ok(), "Parser should return a tree even for invalid syntax");
    ...
    fn test_parse_dsl_whitespace_only() {
        ...
        assert!(result.is_ok(), "Whitespace-only input should be handled gracefully");
    }
...
/* Block comment */"#;
        ...
        assert!(result.is_ok(), "Comments-only input should be handled gracefully");
    }
                ...
                if path.extension().and_then(|s| s.to_str()) == Some("dsl") {
                    ...
                    if filename.starts_with("valid_") {
                        let content = fs::read_to_string(&path)
                            .expect(&format!("Should read file: {:?}", path));
                        
                        let result = parse_dsl(&content);
                        assert!(result.is_ok(), 
                            "Valid sample file {} should parse successfully", filename);
                    }
                }
            }
        }
    ...
    version: 
        ...
        assert!(result.is_ok(), "Parser should handle malformed input gracefully");
    }
    ...
    // Missing closing brace"#;
        ...
        assert!(result.is_ok(), "Parser should handle unclosed braces gracefully");
    }
